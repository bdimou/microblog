...
@bp.route('/', methods =['GET','POST'])                # A decorator modifies the function that follows it. A common pattern with decorators is to use them to register functions as callbacks for certain events
@bp.route('/index',methods =['GET','POST'])            # In this case, the @app.route decorator creates an association between the URL given as an argument and the function 
@login_required
def index():
    form = PostForm()
    if form.validate_on_submit():
        ...
        return redirect(url_for('main.index'))               # Post/Redirect/Get pattern. It avoids inserting posts when a user inadvertently refreshes the pages after submitting a web form.
...
def before_request():
    ...
    g.locale = str(get_locale()) if str(get_locale()) != 'en' else 'EN-US'
